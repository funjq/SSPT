...
def get_model_complexity_info(model, input_res_x,input_res_y,
                              print_per_layer_stat=True,
                              as_strings=True,
                              input_constructor=None, ost=sys.stdout):
    assert type(input_res_x) is tuple
    assert len(input_res_x) >= 2
    flops_model = add_flops_counting_methods(model)
    flops_model.eval()
    flops_model.start_flops_count()
    if input_constructor:
        input = input_constructor(input_res_x)
        _ = flops_model(**input)
    else:
        try:
            batch_x = torch.ones(()).new_empty((1, *input_res_x),
                                             dtype=next(flops_model.parameters()).dtype,
                                             device=next(flops_model.parameters()).device)
            batch_y = torch.ones(()).new_empty((1, *input_res_y),
                                             dtype=next(flops_model.parameters()).dtype,
                                             device=next(flops_model.parameters()).device)
