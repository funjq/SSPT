...
import torch
import yaml
from models.model import SiamUAV_Transformer_Model
...
def copyfiles2checkpoints(opt,path="train.py"):
    # dir_name = './checkpoints'
    dir_name ='./checkpoints'
    #file_name = opt.name   # 假设文件名为 opt.name 加上扩展名
    ...
    if os.path.exists(os.path.join(dir_name, file_name)):
        ...
        while os.path.exists(os.path.join(dir_name, f"{opt.name}_{counter}")):
            counter += 1
        new_file_name = f"{opt.name}_{counter}"
    else:
        # 文件不存在，使用原始文件名
        # new_file_name = file_name
        new_file_name = f"{opt.name}_{0}"
    # 新的文件路径
    new_file_path = os.path.join(dir_name, new_file_name)


    # dir_name = os.path.join('./checkpoints', opt.name)
    dir_name = new_file_path
    if not os.path.isdir(dir_name):
        os.mkdir(dir_name)
    # record every run
    copy_file_or_tree(path,dir_name)
    copy_file_or_tree('test_server.py',dir_name)
    copy_file_or_tree('test_server2.py', dir_name)
    copy_file_or_tree('test_server3.py', dir_name)
    copy_file_or_tree('test_server4.py', dir_name)
